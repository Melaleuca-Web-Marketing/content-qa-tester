...
async function stopCapture() {
  try {
    await fetch('/api/banner/stop', { method: 'POST' });
  } catch (err) {
...
async function resumeCapture() {
  try {
    ...
    const response = await fetch('/api/banner/resume', { method: 'POST' });
    const result = await response.json();
...
async function saveReport() {
  ...
  try {
    ...
    const response = await fetch('/api/banner/report', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (result.ok) {
      setStatusSuccess('Report saved!', `${result.successCount} banners exported to ${result.filename}`);
...
async function loadHistory() {
  try {
    ...
    renderHistory(history.filter(h => h.mode === 'banner'));
  } catch (e) {
...
async function loadReports() {
  try {
    ...
    renderReports(reports.filter(r => r.type === 'banner'));
  } catch (e) {
